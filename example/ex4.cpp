...
int main() {
  ...
  world.setCFM(1.0e-10);
  world.setERP(0.2);

  DaisieUtility util(&sim);

  SphereObjectPack sop = util.createSphere(0.2, 10.0, "sphere");
  SphereObjectPack sop2 = util.createSphere(0.2, 10.0, "sphere2");
  SphereObjectPack sop3 = util.createSphere(0.4, 10.0, "sphere3");
  PlaneObjectPack pop = util.createPlane(0.0, 0.0, 1.0, -1.0, 10.0, 4, "plane");

  sop.body->setPosition(ColumnVector3(0.0, 0.0, 5.8));
  sop2.body->setPosition(ColumnVector3(0.0, 0.0, 5.4));
  sop3.body->setPosition(ColumnVector3(0.0, 0.0, 5.0));

  HingeJoint *joint = world.create<HingeJoint>("hinge");
  joint->connect(sop.body, sop2.body);
  joint->setAnchor(ColumnVector3(0.0, 0.0, 5.4));
  ...
  joint2->setAnchor(ColumnVector3(0.0, 0.0, 5.2));
  ...
  //  STLFileIO::read("bunny.stl", &vertices, &indices);
  //  for (int i = 0; i < vertices.size(); ++i) {
  //    vertices[i] /= 50.0;
  //  }

  TriangleMeshObjectPack top =
      util.createTriangleMesh(vertices, indices, 15.0, "tri");
  top.body->setPosition(ColumnVector3(0.0, 0.0, 4));
  ...
  while (sim.getTime() < 30.0) {
    ...
    camera.repaint();
    cout << sim.getTime() << " " << sop.body->getPosition().transpose() << " "
         << joint->getAngle() << " " << endl;
    //    usleep( 100000 );
  }
