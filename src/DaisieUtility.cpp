...
GLColor randomColor( float Satuation, float Value ) {
  float Hue = 360.0f * (float)rand() / ( (float)RAND_MAX + 1.0f );
  int Hi = (int)( Hue / 60.0f ) % 6;
  float 
    f = Hue / 60.0f - (float)Hi,
    p = Value * ( 1.0f - Satuation ),
    q = Value * ( 1.0f - f * Satuation ),
...
SphereObjectPack DaisieUtility::createSphere( double radius, double density, const string &name_prefix ) {
  ...
  ret.geom = simulator->getSpace().create< SphereGeometry >( name_prefix + "::SphereGeometry" );
...
BoxObjectPack DaisieUtility::createBox( double lx, double ly, double lz, double density, const string &name_prefix ) {
  ...
  ret.geom = simulator->getSpace().create< BoxGeometry >( name_prefix + "::BoxGeometry" );
...
CapsuleObjectPack DaisieUtility::createCapsule( double radius, double length, double density, const string &name_prefix ) {
  ...
  ret.geom = simulator->getSpace().create< CapsuleGeometry >( name_prefix + "::CapsuleGeometry" );
...
CylinderObjectPack DaisieUtility::createCylinder( double radius, double length, double density, const string &name_prefix ) {
  ...
  ret.geom = simulator->getSpace().create< CylinderGeometry >( name_prefix + "::CylinderGeometry" );
  ret.geom->setParams( radius, length );
  ret.geom->connect( ret.body );
...
TriangleMeshObjectPack DaisieUtility::createTriangleMesh( const vector< ColumnVector3 > &vertices,
  ...
  ret.geom = simulator->getSpace().create< TriangleMeshGeometry >( name_prefix + "::TriangleMeshGeometry" );
  ret.geom->build( vertices, indices );
  ret.geom->connect( ret.body );
  
  ret.shape = this->create< GLTriangleMeshShape >( name_prefix + "::GLTriangleMeshShape" );
...
PlaneObjectPack DaisieUtility::createPlane( double a, double b, double c, double d, double size, int num, const string &name_prefix ) {

  PlaneObjectPack ret;

  ret.geom = simulator->getSpace().create< PlaneGeometry >( name_prefix + "::PlaneGeometry" );
  ret.geom->setParams( a, b, c, d );
  
  ret.shape = this->create< GLPlaneShape >( name_prefix + "::GLPlaneShape" );
  ret.shape->setParams( a, b, c, d, size, num, ColumnVector3( a, b, c ).normalize() );
  GLColor color = randomColor( 0.5, 1.0 );
  ret.shape->setAmbient( color );
  ret.shape->setDiffuse( color );
...
RayObjectPack DaisieUtility::createRay( double length, const string &name_prefix ) {

  RayObjectPack ret;

  ret.geom = simulator->getSpace().create< RayGeometry >( name_prefix + "::RayGeometry" );
...
void DaisieUtility::syncShape() {
  ...
  {
    list< pair< Body*, GLShape* > >::iterator 
      it = bodyShapeList.begin(),
  ...
  {
    list< pair< Geometry*, GLShape* > >::iterator 
      it = geomShapeList.begin(),
