...
Geometry::Geometry( Space &space, dGeomID id, const string &name ) 
  ...
  //  dGeomTransformSetCleanup( _id, 1 );    // geomIDがdestroy時に, transferedGeomIDもdestroy
...
void Geometry::setFeedbackEnable( bool onoff ) {
  keepColData = onoff;
}

bool Geometry::isFeedbackEnabled() const {
  return keepColData;
}
...
void RayGeometry::setLength( double length ) {
...
TriangleMeshGeometry::TriangleMeshGeometry( Space &space, const string &name )
  : Geometry( space, dCreateTriMesh( space.id(), dGeomTriMeshDataCreate(), NULL, NULL, NULL ), name ),
...
void TriangleMeshGeometry::build( const vector< ColumnVector3 > &vertices,
  ...
  dGeomTriMeshDataBuildDouble( Data,
                               vertexBuf, 3 * sizeof( double ), vertexNum,
