...
public:
  ...
  NearCallback( Simulator *simulator, int maxContacts, const dSurfaceParameters &surfaceParams );
  ~NearCallback();
...
void NearCallback::_nearCallbackFunc ( void *data, dGeomID geomID1, dGeomID geomID2 ) {
  ( *static_cast< NearCallback* >( data ) )( geomID1, geomID2 );
}

NearCallback::NearCallback( Simulator *simulator, int maxContacts, const dSurfaceParameters &surfaceParams )
  : simulator( simulator ),
...
void NearCallback::operator()( dGeomID geomID1, dGeomID geomID2 ) {
  ...
  Geometry
    *g1 = static_cast< Geometry* >( dGeomGetData( geomID1 ) ),
  ...
  dBodyID 
    body1 = dGeomGetBody( geomID1 ),
    body2 = dGeomGetBody( geomID2 );

  dWorldID worldID = simulator->getWorld().id();

  dContact contact[ maxContacts ];
    
  int colNum = dCollide( geomID1, geomID2, maxContacts, &contact[ 0 ].geom, sizeof( dContact ) );
    
  bool fbEnabled = ( g1->isFeedbackEnabled() || 
		     g2->isFeedbackEnabled() );
  ...
  for ( int i = 0; i < colNum; ++i ) {
    ...
    if ( dGeomGetClass( geomID1 ) != dRayClass && 
	 dGeomGetClass( geomID2 ) != dRayClass ){
      ...
      jointID = dJointCreateContact( worldID, 
				     contactJointGroupID, 
				     &contact[ i ] );
...
void NearCallback::setCollisionFeedback() {
  ...
  for ( unsigned int i = 0; i < size ; ++i ){
	...
	cdata.relPos = getRelFromGlobal( cfb->contactGeom.g1, cfb->contactGeom.pos );
	cdata.relNormal = getRelFromGlobal( cfb->contactGeom.g1, cfb->contactGeom.normal );

	cdata.absForce.fill( cfb->jointFB.f1 );
	cdata.absTorque.fill( cfb->jointFB.t1 );
	
	cdata.relForce = getRelFromGlobal( cfb->contactGeom.g1, cfb->jointFB.f1 );
	cdata.relTorque = getRelFromGlobal( cfb->contactGeom.g1, cfb->jointFB.t1 );
      }
	...
	cdata.relPos = getRelFromGlobal( cfb->contactGeom.g2, cfb->contactGeom.pos );
	cdata.relNormal = getRelFromGlobal( cfb->contactGeom.g2, cfb->contactGeom.normal );

	cdata.absForce.fill( cfb->jointFB.f2 );
	cdata.absTorque.fill( cfb->jointFB.t2 );
	
	cdata.relForce = getRelFromGlobal( cfb->contactGeom.g1, cfb->jointFB.f2 );
	cdata.relTorque = getRelFromGlobal( cfb->contactGeom.g1, cfb->jointFB.t2 );
      }
...
Simulator::Simulator( int maxContacts )
  : maxContacts( maxContacts ),
    simTime( 0.0 ) {
  dInitODE();
}
...
void Simulator::removeIgnoredGeometryPair( dGeomID geom1, dGeomID geom2 ) {
  ...
  {
    pair< multimap< dGeomID, dGeomID >::iterator, multimap< dGeomID, dGeomID >::iterator >
...
bool Simulator::isIgnored( dGeomID geom1, dGeomID geom2 ) const {
  ...
  pair< multimap< dGeomID, dGeomID >::const_iterator, multimap< dGeomID, dGeomID >::const_iterator >
    p = ignoredGeomPair.equal_range( geom1 );
...
void Simulator::update( double timeStep ) {
  ...
  NearCallback *nearCallback = new NearCallback( this, maxContacts, surfaceParams );
