...
Joint::Joint( dJointID id, const string &name ) 
  : DaisieObject( name ), 
    _id( id ) {
...
Body* Joint::getConnectedBody( bool isFirst ) const {
  return static_cast< Body* >( dBodyGetData( dJointGetBody( _id, ( ( isFirst )? 0: 1 ) ) ) );
}
...
void Hinge2Joint::setFudgeFactor( double fudgeFactor, bool isFirst ) {
  dJointSetHinge2Param( id(), paramOffset( dParamFudgeFactor, isFirst ), fudgeFactor );
}
...
void Hinge2Joint::addTorque( double torque, bool isFirst ) {
  dJointAddHinge2Torques( id(), ( ( isFirst )? torque: 0.0 ), ( ( isFirst )? 0.0: torque ) );
}
...
void SliderJoint::addForce( double force ) {
...
void UniversalJoint::setFudgeFactor( double fudgeFactor, bool isFirst ) {
  dJointSetUniversalParam( id(), paramOffset( dParamFudgeFactor, isFirst ), fudgeFactor );
}
...
void UniversalJoint::addTorque( double torque, bool isFirst ) {
  dJointAddUniversalTorques( id(), ( ( isFirst )? torque: 0.0 ), ( ( isFirst )? 0.0: torque ) );
}
