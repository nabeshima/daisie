...
void Body::setRotation( const RotationMatrix& R ) {
  dMatrix3 tmp;
  tmp[ 0 ] = R( 0, 0 ); tmp[ 1 ] = R( 0, 1 ); tmp[  2 ] = R( 0, 2 ); tmp[  3 ] = 0.0;
  tmp[ 4 ] = R( 1, 0 ); tmp[ 5 ] = R( 1, 1 ); tmp[  6 ] = R( 1, 2 ); tmp[  7 ] = 0.0;
  tmp[ 8 ] = R( 2, 0 ); tmp[ 9 ] = R( 2, 1 ); tmp[ 10 ] = R( 2, 2 ); tmp[ 11 ] = 0.0;
  dBodySetRotation( _id, tmp );
}

void Body::setQuaternion( const Quaternion& q ) {
  dBodySetQuaternion( _id, q.data() );  // dQuaternion is array of double whose size is 4.
}
...
RotationMatrix Body::getRotation() const {
  ...
  return RotationMatrix( tmp[ 0 ], tmp[ 1 ], tmp[  2 ], 
			 tmp[ 4 ], tmp[ 5 ], tmp[  6 ],
			 tmp[ 8 ], tmp[ 9 ], tmp[ 10 ] );
}
...
void Body::setMass( const Mass &m ) {
  dBodySetMass( _id, &m );
}
...
void Body::addForceAtPoint( const ColumnVector3 &f,
			    const ColumnVector3 &p ) {
  dBodyAddForceAtPos( _id, 
		      f( 0 ), f( 1 ), f( 2 ), 
		      p( 0 ), p( 1 ), p( 2 ) );
}
